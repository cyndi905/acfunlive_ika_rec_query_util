...
def get_ika_cut_id(author_id):
  if str(author_id) in live_dic:
    live_id = live_dic[str(author_id)]['liveId']
...
def save_ika_data(res, author_id,live_id ,is_err_save_opt = False):
  r_json = res.json()
  authorName = live_dic[str(author_id)]['authorName']
  try:
    ...
    iKaUrl = '该主包未开启爱咔录像权限'
    iKaCutId = 0
  title = live_dic[str(author_id)]['title']
  startLiveTime = live_dic[str(author_id)]['startLiveTime']
  uid = int(author_id)
